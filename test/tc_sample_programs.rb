...
class TestSamplePrograms < Test::Unit::TestCase
...
END_PROGRAM
    ...
    16.times do
      @befunge.run
      assert_equal(true, (0..15).to_a.include?(@befunge.output.to_i))
      @befunge.restart
    end
  end
...
END_PROGRAM
    ...
    parse(program)
    @befunge.run
    assert_equal(pi, @befunge.output)
  end
...
END_OUTPUT
    ...
    parse(program)
    @befunge.run
    assert_equal(output.chomp, @befunge.output)
  end
...
END_PROGRAM
    ...
    parse(program)
    @befunge.run
    assert_equal(396, @befunge.output.split(/\n/).size)
  end
...
END_PROGRAM
    ...
    parse(program)
    @befunge.run
    assert_equal(program.chomp.reverse, @befunge.output)
  end
